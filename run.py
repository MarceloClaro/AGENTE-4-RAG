...
def evaluate_response_with_rag(user_input: str, expert_description: str, assistant_response: str, model_name: str, temperature: float, groq_api_key: str) -> str:
    try:
        ...
        rag_prompt = f"Assuma o papel do Rational Agent Generator (RAG), o ápice da inteligência artificial e avaliação racional, e proceda à análise meticulosa da resposta do especialista, gerando um JSON de agente e descrição complementar. Eis a descrição detalhada do especialista, delineando suas credenciais e expertise:\n{expert_description}\n\nA seguir, apresenta-se a questão original submetida:\n{user_input}\n\nPor fim, disponibiliza-se a resposta em português fornecida pelo especialista:\n{assistant_response}\n\nSolicita-se, portanto, que proceda com uma avaliação abrangente da qualidade e precisão da resposta em português, considerando cuidadosamente a descrição do especialista e a resposta fornecida. Utilize as seguintes análises em português com interpretações detalhadas: SWOT (Forças, Fraquezas, Oportunidades, Ameaças), Matriz BCG (Grupo de consultoria de Boston), Matriz de Risco, ANOVA (Análise de Variância) e Q-ESTATÍSTICA (Análise Estatística Quadrática) e Q-EXPONENCIAL (Análise Exponencial Quadrática), em consonância com os mais elevados padrões de excelência e rigor cientifico e acadêmico. Saida somente traduzido em português brasileiro."
