...
def fetch_assistant_response(user_input: str, model_name: str, temperature: float, agent_selection: str, groq_api_key: str) -> Tuple[str, str]:
    ...
    try:
        ...
        if agent_selection == "Escolha um especialista...":
            phase_one_prompt = f"Assuma o papel de um especialista em engenharia de prompt e multidisciplinar altamente qualificado e rigor técnico científico. Apresente obrigatoriamente no formato 'markdown' para códigos em python e suas diversas bibliotecas, com comentarios em cada linha minuciosamente e didaticamente. Ao analisar atentamente a solicitação apresentada, é essencial identificar os critérios que definem o perfil do especialista mais adequado para abordar a questão com maestria e profundidade. Inicialmente, é fundamental estabelecer o título que melhor reflete a expertise necessária para oferecer uma resposta completa, profundas e esclarecedora. Após essa identificação, segue-se com uma descrição precisa e meticulosa das habilidades e qualificações essenciais desse especialista evitando vieis. Inicie a resposta com o título do especialista, seguido de um ponto ['.'], e prossiga com uma exposição concisa, didatica e minucioso com muita profundidade, porém abrangente, das características e competências que o qualificam para lidar com o questionamento apresentado: {user_input}. Esta análise cuidadosa é crucial para garantir que o especialista selecionado possua o conhecimento mediculoso, profundo e a experiência necessários para abordar a questão de forma completa e satisfatória com uma acuracia 10,0 , atendendo aos mais altos padrões de excelência profissional, cientifica e acadêmica. Em caso de código e calculos, apresente obrigatoriamente no formato 'markdown', com comentarios em cada linha minuciosamente."
        ...
        phase_two_prompt = f"No papel de {expert_title}, um especialista amplamente reconhecido e respeitado em sua área de atuação, sendo um expert e phd na área , permita-me fornecer uma resposta abrangente e meticulosamente com muita profundidade conceitual teorico e prática elaborada para a seguinte questão, apresentada de forma clara, detalhada, expandida, didaticamente acessivel e concisa: {user_input}. Neste contexto, considerando minha experiência de longa data e profundo conhecimento na disciplina em questão, é imperativo abordar cada aspecto com o devido cuidado e rigor tecnico-cientifico-acadêmico. Desse modo, delinearei os principais elementos a serem considerados e profundos, fornecendo uma análise detalhada e embasada em evidências e evitando vieis. Ao final deste processo, objetiva-se apresentar uma resposta completa e satisfatória, alinhada com os mais elevados padrões de excelência acadêmica e profissional, para atender às necessidades específicas do questionamento apresentado. Apresente obrigatoriamente no formato 'markdown', com comentarios em cada linha minuciosamente. Mantendo um padrão de escrita com 10 parágrafos, cada parágrafos com 4 frases, cada frases com uma virgula, mantendo sempre as melhores práticas didaticas e pedagogicas aristótelicas."
